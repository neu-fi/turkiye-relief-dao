...
import { LinkIcon, ClipboardIcon } from "@heroicons/react/20/solid";
...
export default function OrganizationCard({
	...
	const optionToIconUrl = (option: Option) => option.type === "cryptocurrency" ? icons.cryptocurrencies[option.name] : icons[option.type];
			...
			<div className="ml-4 mt-4">
				<div className="flex items-center">
					<div className="flex-shrink-0">
						<img className="h-16 w-16 mix-blend-multiply" src={organization.logoUrl} />
					</div>
					<div className="ml-4">
						...
						<div className="space-x-2">
							{organization.categories.map((category: any) => (
								<span className={classNames( "inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium", categoryDetails[category].classes)}>
		...
		return (
			<div className="mt-4 flex grow justify-center sm:justify-end">
				<Link href={organization.twitterUrl}>
					<button type="button" className="relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50">
				...
				<Link href={organization.websiteUrl}>
					<button type="button" className="relative ml-3 inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50">
						<LinkIcon className="-ml-1 mr-2 h-5 w-5 text-gray-400" aria-hidden="true" />
						<span>Website</span>
					</button>
				</Link>
				<button type="button" onClick={() => setExpanded(!expanded)} className="relative ml-3 inline-flex items-center rounded-md border border-gray-300 bg-white px-2 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50">
					<div className={`${expanded ? "rotate-180" : ""} h-5 w-5 text-gray-400 fill-current transition-all duration-300`} aria-hidden="true">
						<ChevronDownIcon />
					</div>
				</button>
			</div>
	...
	const createOptionActions = (option: any) => {
		return (
			<td className="px-2 sm:px-3 md:space-x-1 text-sm text-gray-400 grow inline-flex place-items-center justify-end">
				...
				<button type="button" onClick={() => onCopy(option)} className={classNames(
	...
	const createOption = (option: Option, index: number) => {
		return (
			<tr key={index} className="flex justify-items-end w-full place-items-center py-2 sm:py-5">
							...
							<div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
								<table className="flex flex-col divide-y divide-gray-300">
									<tbody className="divide-y divide-gray-200 bg-white">
										{organization.options.map((option: Option, index: number) => isOptionFiltered(option) && createOption(option, index))}
									</tbody>
								</table>
							</div>
						</div>
